...
def load_stocks():
    ...
    # Preserve the complete StockStory universe context. Multiple rows for a
    # symbol are expected because one stock can qualify through several rules.
    # Aggregate them before scanning so RSI runs once per unique symbol.
    for col in ["Source", "Period", "Sector", "Rank", "Favorite Reasons", "Favorite Notes"]:
        if col not in df.columns:
            df[col] = ""
        df[col] = df[col].fillna("").astype(str).str.strip()

    grouped = []
    for symbol, rows in df[df["Symbol"] != ""].groupby("Symbol", sort=False):
        ...
        grouped.append({
            "Symbol": symbol,
            "Tag": join_unique("Source"),
            "Universe Sources": join_unique("Source"),
            "Universe Periods": join_unique("Period"),
            "Universe Sectors": join_unique("Sector"),
            "Universe Ranks": join_unique("Rank"),
            "Favorite Reasons": join_unique("Favorite Reasons"),
            "Favorite Notes": join_unique("Favorite Notes"),
        })
...
def process_stock(symbol, tag="", check_15m=True, universe_meta=None):
    ...
    return {
        ...
        "Symbol": symbol,
        "Tag": tag,
        "Universe Sources": universe_meta.get("Universe Sources", tag),
        "Universe Periods": universe_meta.get("Universe Periods", ""),
        "Universe Sectors": universe_meta.get("Universe Sectors", ""),
        "Universe Ranks": universe_meta.get("Universe Ranks", ""),
        "Favorite Reasons": universe_meta.get("Favorite Reasons", ""),
        "Favorite Notes": universe_meta.get("Favorite Notes", ""),
        "Chartink URL": f"https://chartink.com/stocks/{quote(symbol, safe='')}.html",
...
def main():
    ...
    if full_hourly:
        ...
        for n,(_,row) in enumerate(stocks.iterrows(),1):
            symbol=row["Symbol"]; tag=row.get("Tag",""); meta=row.to_dict()
            print(f"\n[{n}/{len(stocks)}] {symbol} [HOURLY]")
            try:
                result=process_stock(symbol,tag,check_15m=False,universe_meta=meta)
        ...
        for n,(_,row) in enumerate(monitored_rows.iterrows(),1):
            ...
            try:
                result=process_stock(symbol,tag,check_15m=True,universe_meta=meta)
                if result is not None:
